...
import { Bot, Brain, Cloud, Code2, Layers, PenTool, ArrowUpRight, type LucideIcon } from 'lucide-react';
      ...
      <div className="relative mx-auto w-full max-w-[1320px] px-5 sm:px-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <SectionHeading
            ...
            title="What We Build"
            copy="Six disciplines, one delivery team. We assemble the capability mix your product actually needs."
          />
        </div>
            ...
            return (
              <Reveal key={service.id} delay={(i % 3) * 0.08}>
                <Link
                  ...
                  className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-hairline bg-navy/40 p-7 transition-all duration-500 hover:-translate-y-1"
                  onMouseEnter={(e) => {
                    e.currentTarget.style.borderColor = `${service.accent}66`;
                    e.currentTarget.style.boxShadow = `0 28px 60px -30px ${service.accent}80`;
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <div className="relative flex items-start justify-between">
                    <span
                      className="inline-flex h-11 w-11 items-center justify-center rounded-xl transition-colors duration-500"
                      style={{ backgroundColor: `${service.accent}1A` }}
                    >
                      <Icon className="h-5 w-5" style={{ color: service.accent }} strokeWidth={1.6} />
                    </span>
                  ...
                  <span
                    className="relative mt-5 inline-flex items-center gap-1 text-[0.82rem] font-medium opacity-0 transition-opacity group-hover:opacity-100"
